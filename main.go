...
func main() {
	...
	domainSpecified := *domain != ""

	// Determine if we need to generate self-signed certs
	if (!certSpecified || !keySpecified) && !domainSpecified {
	...
	if *redirectHTTP {
		...
		go func() {
			log.Printf("Also redirecting https requests on port 80 to https requests on %s", redirectURL)
	...
	if domainSpecified {
		...
		log.Println("not domainSpecified")
